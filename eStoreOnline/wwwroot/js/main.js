...
function reloadCart() {
    let cartCountSection = $('#CartCount');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            cartCountSection.text(response.cartDetail.length);
            if (response.cartDetail.length === 0) {
                cartCountSection.hide();
                headerCart.html('<p class="text-center m-5">No items in cart</p>');
            } else {
                cartCountSection.show();
                headerCart.html('<ul class="mini-products-list"></ul>');
                response.cartDetail.forEach(function (item) {
                    let product = `<li class="item">
                                <a class="product-image" href="#">
                                    <img src="${item.imageUrl}" alt="${item.productName}" title=""/>
                                </a>
                                <div class="product-details">
                                    <a onclick="removeItemFromCart(${item.productId})" class="remove">
                                        <i class="anm anm-times-l" aria-hidden="true"></i>
                                    </a>
                                    <a class="pName" href="/Product/Detail/${item.id}">${item.productName}</a>
                    ...
                    headerCart.find('.mini-products-list').append(product);
                });

                headerCart.find('.mini-products-list').append(`<div class="total">
                            <div class="total-in">
                                <span class="label">Cart Subtotal:</span>
                                <span class="product-price">
                                    <span class="money">$${response.totalAmount.toFixed(2)}</span>
                                </span>
                            </div>
                            <div class="buttonSet text-center">
                                <a href="/Cart/Index" class="btn btn-secondary btn--small">View Cart</a>
                                <a href="/Cart/Index" class="btn btn-secondary btn--small">Checkout</a>
                            </div>
                        </div>`);
            }
        },
        error: function (jqXHR, exception) {
            if (jqXHR.status === 401) {
                headerCart.html('<p class="text-center m-5">Please sign in to add item to cart</p>');
            }
        }
    });
}
